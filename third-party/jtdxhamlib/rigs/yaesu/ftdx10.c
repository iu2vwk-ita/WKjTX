...
/*
 * Yaesu FTX-1 (Field/Optima) SDR portable transceiver.
 * CAT command set is the newcat family, identical to the FTDX-10, so this
 * caps struct is a clone of ftdx10_caps sharing the same newcat handlers,
 * priv data, ext tokens/levels and frequency/mode tables. Only the model
 * number and display name differ. In newcat.c the FTX1 model is folded into
 * the is_ftdx10 detection so it takes exactly the FTDX-10 code paths.
 */
const struct rig_caps ftx1_caps =
{
